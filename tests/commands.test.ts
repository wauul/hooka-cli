...
it("lists and adds endpoints including the signing secret",async()=>{
  ...
  expect((await run(["endpoints","add",endpoint.url,"--events","order.shipped, order.cancelled"])).output).toContain("test-signing-secret");
  expect(requests.at(-1)?.body).toEqual({url:endpoint.url,eventTypes:["order.shipped","order.cancelled"]});
});
it("sends inline JSON and waits for the original generation",async()=>{const result=await run(["send","--type","order.shipped","--payload",'{"orderId":123}']);expect(result.output).toContain("DELIVERED");expect(requests[0].body).toEqual({type:"order.shipped",payload:{orderId:123}});expect(requests[1].path).toBe("/api/v1/events/ev?generation=0");});
it("reads payload files and can skip waiting",async()=>{const file=join(dir,"payload.json");await writeFile(file,'{"unicode":"你好"}');await run(["send","--type","test","--payload-file",file,"--no-wait","--idempotency-key","mine"]);expect(requests).toHaveLength(1);expect(requests[0].body).toEqual({type:"test",payload:{unicode:"你好"},idempotencyKey:"mine"});});
it("prompts for omitted event fields",async()=>{const result=await run(["send","--no-wait"],["test","null"]);expect(result.ask).toHaveBeenCalledTimes(2);expect(requests[0].body).toEqual({type:"test",payload:null});});
...
it.each([
  ["send","--type","test","--payload","{}","--payload-file","x"],
  ["send","--type","bad space","--payload","{}"],
  ["send","--type","test","--payload","broken"],
  ["send","--type","test","--payload-file","missing.json"],
  ["endpoints","add",endpoint.url,"--events",","],
